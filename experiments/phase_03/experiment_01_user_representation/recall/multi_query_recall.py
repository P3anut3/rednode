...
def search_cached(index: faiss.Index, queries: np.ndarray, topk: int, cache_dir: Path,
    ...
    if old is None:
        ...
        start_offset = 0
        meta = {"complete": False, "shape": list(shape), "topk": topk,
                "queries": len(queries), "batch_size": batch_size, "next_offset": 0,
                "query_seconds": 0.0}
        meta_path.write_text(json.dumps(meta, indent=2))
    else:
        scores = np.memmap(score_path, mode="r+", dtype=np.float32, shape=shape)
        rows = np.memmap(row_path, mode="r+", dtype=np.int32, shape=shape)
        start_offset = min(int(old.get("next_offset", 0)), len(queries))
        meta = old
    start = time.perf_counter()
    prior_seconds = float(meta.get("query_seconds", 0.0))
    for offset in range(start_offset, len(queries), batch_size):
        ...
        meta.update({"complete": False, "next_offset": offset + len(batch),
                     "query_seconds": prior_seconds + time.perf_counter() - start})
        meta_path.write_text(json.dumps(meta, indent=2))
    scores.flush(); rows.flush()
    meta.update({"complete": True, "next_offset": len(queries),
                 "query_seconds": prior_seconds + time.perf_counter() - start})
    meta_path.write_text(json.dumps(meta, indent=2))
...
def _candidate_union(rows_by_interest: list[np.ndarray], budget_mode: str,
    ...
    if budget_mode == "practical":
        primary = np.unique(np.concatenate([rows[:per_interest_budget]
                                            for rows in rows_by_interest]))
        # The nominal budget is Top-M per interest. Rows beyond M are only a
        # post-filter buffer: collapsed interests can otherwise yield fewer
        # than final_k candidates after recent-history removal.
        selected = list(map(int, primary[primary >= 0]))
        seen = set(selected)
        eligible = sum(int(note_ids[row]) not in blocked for row in selected)
        depth = per_interest_budget
        while eligible < final_k and any(depth < len(rows) for rows in rows_by_interest):
            for rows in rows_by_interest:
                if depth >= len(rows):
                    continue
                row = int(rows[depth])
                ...
                selected.append(row)
                seen.add(row)
                if int(note_ids[row]) not in blocked:
                    eligible += 1
                    if eligible == final_k:
                        break
            depth += 1
