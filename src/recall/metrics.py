...
def _target_for_segment(request: TestRequest, segment: str, warm_items: set[int], warm_users: set[int],
                        clicked_items: set[int] | None = None) -> set[int] | None:
...
def evaluate_rankings(
    requests: Sequence[TestRequest], rankings: Sequence[Sequence[int]],
    warm_items: set[int], warm_users: set[int], method: str,
    ...
    accum = {s: {"recalls": {k: [] for k in KS}, "hits": {k: [] for k in KS},
                 "rr100": [], "rr500": [], "ranks": [], "misses": 0, "eligible": 0}
             for s in segments}
    per_request = []
    for request, ranking_values in zip(requests, rankings):
        ...
        overall_hit_ranks = [rank_by_note[note] for note in overall_truth if note in rank_by_note]
        ...
        for segment in segments:
            target = _target_for_segment(request, segment, warm_items, warm_users, clicked_items)
            if not target:
                continue
            bucket = accum[segment]
            bucket["eligible"] += 1
            target_hit_ranks = [rank_by_note[note] for note in target if note in rank_by_note]
            first_hit = min(target_hit_ranks, default=None)
            if first_hit is None:
                bucket["misses"] += 1
            else:
                bucket["ranks"].append(first_hit)
            bucket["rr100"].append(1.0 / first_hit if first_hit is not None and first_hit <= 100 else 0.0)
            bucket["rr500"].append(1.0 / first_hit if first_hit is not None and first_hit <= 500 else 0.0)
            for k in KS:
    ...
    for segment, bucket in accum.items():
        ...
        row: dict[str, float | int] = {
            "method": method, "segment": segment, "eligible_requests": eligible,
            "overall_miss_rate@500": bucket["misses"] / eligible if eligible else float("nan"),
            "mean_first_hit_rank_on_hits": float(np.mean(ranks)) if ranks else float("nan"),
            "median_first_hit_rank_on_hits": float(np.median(ranks)) if ranks else float("nan"),
        ...
        for k in KS:
            row[f"Recall@{k}"] = float(np.mean(bucket["recalls"][k])) if eligible else float("nan")
            row[f"HitRate@{k}"] = float(np.mean(bucket["hits"][k])) if eligible else float("nan")
        metrics[segment] = row
